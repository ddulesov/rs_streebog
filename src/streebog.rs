    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {   
        unsafe {
            for x in self.xmm.iter(){
                write!(f, "\n{:x} {:x}", _mm256_extract_epi64(*x, 0) , _mm256_extract_epi64(*x,  1) )? 
            }
            Ok(())
        }
    }
...
impl Clone for B512 {
    fn clone(&self) -> Self {
        unsafe{ B512{ b64: self.b64.clone() } }
    }
}
...
impl From<&[u8]> for B512{
    fn from(data: &[u8]) -> Self {
        ...
        unsafe { copy_nonoverlapping(data.as_ptr(), b8.as_mut_ptr(), 64 ); };
        B512{ b8:b8 }
    }
}
...
impl Default for B512{
    fn default() -> Self{ B512{ b8: unsafe { MaybeUninit::uninit().assume_init() }  } }
}
...
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "StreeBog{} buffer:{:?} hash:{:?}", T::SIZE,  &self.buffer, &self.h ) 
    }
}
...
impl<T> StreeBog<T> 
    ...
    pub fn new()->StreeBog<T> {
        let mut o = StreeBog{h: Default::default(), buffer: Default::default(),
        n:Default::default(), sigma: Default::default(),  bufsize:0, phantom: PhantomData };  //  
        o.reset();
        o
    }
